...
class DeteksiDiniService
{
    ...
    public function hitungSkorRisiko(int $siswaId, string $tahunAjaran, string $semester): DeteksiDiniSiswa
    {
        ...
        $skor += min($totalLaporan * 5, 20);              // maks 20 poin dari jumlah laporan

        if ($laporanKritis > 0) {
            $skor += $laporanKritis * 15;                  // 15 poin per laporan kritis
        ...
        // Asesmen kesehatan mental — cek jawaban berindikasi masalah
        ...
        if (!$asesmenSelesai) {
            $skor += 3; // Sedikit tambah skor karena data tidak lengkap
