...
@section('title', 'Edit Agenda - Schoolify Admin')
...
<div class="space-y-6 animate-fadeInUp">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-outfit font-extrabold text-xl text-[var(--text-primary)]">Form Edit Agenda</h2>
                <p class="text-xs text-[var(--text-secondary)] mt-0.5">Perbarui data agenda: {{ $agenda->title }}</p>
            </div>
        </div>
    ...
    <div class="neo-flat rounded-3xl p-6 relative z-10">
        <form action="{{ route('admin.agendas.update', $agenda->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="space-y-2 md:col-span-2">
                    ...
                    <input type="text" name="title" class="w-full neo-input py-3 px-4 text-sm font-semibold" value="{{ old('title', $agenda->title) }}" placeholder="Contoh: Ujian Tengah Semester" required>
                ...
                <div class="space-y-2 relative">
                    ...
                    <div class="relative">
                        <select name="type" class="w-full neo-input appearance-none py-3 px-4 text-sm font-semibold cursor-pointer" required>
                            ...
                            <option value="uts" {{ old('type', $agenda->type) == 'uts' ? 'selected' : '' }}>📝 UTS (Ujian Tengah Semester)</option>
                            <option value="uas" {{ old('type', $agenda->type) == 'uas' ? 'selected' : '' }}>📚 UAS (Ujian Akhir Semester)</option>
                ...
                <div class="space-y-2">
                    ...
                    <input type="date" name="start_date" class="w-full neo-input py-3 px-4 text-sm font-semibold" value="{{ old('start_date', $agenda->start_date->format('Y-m-d')) }}" required>
                ...
                <div class="space-y-2">
                    ...
                    <input type="date" name="end_date" class="w-full neo-input py-3 px-4 text-sm font-semibold" value="{{ old('end_date', $agenda->end_date ? $agenda->end_date->format('Y-m-d') : '') }}">
                ...
                <div class="space-y-2">
                    ...
                    <input type="time" name="start_time" class="w-full neo-input py-3 px-4 text-sm font-semibold" value="{{ old('start_time', $agenda->start_time) }}">
                ...
                <div class="space-y-2">
                    ...
                    <input type="time" name="end_time" class="w-full neo-input py-3 px-4 text-sm font-semibold" value="{{ old('end_time', $agenda->end_time) }}">
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <input type="text" name="location" class="w-full neo-input py-3 px-4 text-sm font-semibold" value="{{ old('location', $agenda->location) }}" placeholder="Contoh: Aula Sekolah">
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <textarea name="description" class="w-full neo-input py-3 px-4 text-sm font-semibold min-h-[100px] resize-y" placeholder="Deskripsi atau catatan tambahan">{{ old('description', $agenda->description) }}</textarea>
            ...
            <div class="flex justify-between items-center pt-6 border-t border-[var(--shadow-dark)]/10 mt-8">
                <button type="button" onclick="document.getElementById('deleteForm').submit();" class="neo-btn px-4 py-2.5 rounded-xl text-sm font-bold text-red-500 border border-red-500/20 hover:bg-red-500/10 flex items-center gap-2 transition-colors">
                    <i data-lucide="trash-2" class="w-4 h-4"></i> Hapus Agenda
                </button>
        ...
        <form id="deleteForm" action="{{ route('admin.agendas.delete', $agenda->id) }}" method="POST" class="hidden" onsubmit="return confirm('Hapus agenda ini secara permanen?')">@csrf @method('DELETE')</form>
    </div>
</div>
@endsection