...
@section('title', 'Tambah Jadwal - Schoolify Admin')
...
<div class="max-w-4xl mx-auto animate-fadeInUp">
    ...
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h3 class="font-outfit font-black text-2xl text-[var(--text-primary)]">Buat Jadwal Baru</h3>
                <p class="text-xs text-[var(--text-secondary)] font-bold uppercase tracking-widest">Manajemen Kurikulum & Kelas</p>
            </div>
        </div>
    ...
    <div class="neo-flat rounded-[2.5rem] overflow-hidden">
        <div class="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 px-8 py-6 border-b border-[var(--shadow-dark)]/5">
            <div class="flex items-center gap-3">
                <i data-lucide="form-input" class="w-5 h-5 text-indigo-500"></i>
                <h4 class="font-black text-sm text-[var(--text-primary)] uppercase tracking-wider">Form Informasi Jadwal</h4>
            </div>
        </div>
        ...
        <form action="{{ route('admin.jadwal.store') }}" method="POST" class="p-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    <label class="flex items-center gap-2 text-xs font-black text-[var(--text-secondary)] uppercase tracking-widest ml-1">
                        <i data-lucide="door-open" class="w-4 h-4 text-indigo-500"></i> Pilih Kelas <span class="text-rose-500">*</span>
                    </label>
                ...
                <div class="space-y-3">
                    <label class="flex items-center gap-2 text-xs font-black text-[var(--text-secondary)] uppercase tracking-widest ml-1">
                        <i data-lucide="user-check" class="w-4 h-4 text-indigo-500"></i> Pilih Guru Pengampu <span class="text-rose-500">*</span>
                    </label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    <label class="flex items-center gap-2 text-xs font-black text-[var(--text-secondary)] uppercase tracking-widest ml-1">
                        <i data-lucide="book-open" class="w-4 h-4 text-indigo-500"></i> Mata Pelajaran <span class="text-rose-500">*</span>
                    </label>
                    <div class="relative">
                        <select name="mata_pelajaran" id="mapelSelect" class="w-full neo-input py-3.5 px-5 font-bold text-sm appearance-none cursor-pointer" required>
                            ...
                            @endforeach
                            <option value="Lainnya" {{ old('mata_pelajaran') == 'Lainnya' ? 'selected' : '' }}>Lainnya (Ketik Manual)</option>
                        </select>
                    </div>
                    <input type="text" name="mata_pelajaran_lain" id="mapelLainInput" class="w-full neo-input py-3.5 px-5 font-bold text-sm mt-3 hidden animate-fadeInUp" placeholder="Ketik Mata Pelajaran Manual...">
                </div>

                <div class="space-y-3">
                    <label class="flex items-center gap-2 text-xs font-black text-[var(--text-secondary)] uppercase tracking-widest ml-1">
                        <i data-lucide="calendar" class="w-4 h-4 text-indigo-500"></i> Hari <span class="text-rose-500">*</span>
                    </label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-3">
                        <label class="flex items-center gap-2 text-xs font-black text-[var(--text-secondary)] uppercase tracking-widest ml-1">
                            <i data-lucide="clock-3" class="w-4 h-4 text-indigo-500"></i> Jam Mulai <span class="text-rose-500">*</span>
                        </label>
                        <input type="time" name="jam_mulai" value="{{ old('jam_mulai') }}" class="w-full neo-input py-3.5 px-5 font-bold text-sm" required>
                    </div>
                    <div class="space-y-3">
                        <label class="flex items-center gap-2 text-xs font-black text-[var(--text-secondary)] uppercase tracking-widest ml-1">
                            <i data-lucide="clock-9" class="w-4 h-4 text-rose-500"></i> Jam Selesai <span class="text-rose-500">*</span>
                        </label>
                        <input type="time" name="jam_selesai" value="{{ old('jam_selesai') }}" class="w-full neo-input py-3.5 px-5 font-bold text-sm" required>
                    </div>
                </div>

                <div class="space-y-3">
                    <label class="flex items-center gap-2 text-xs font-black text-[var(--text-secondary)] uppercase tracking-widest ml-1">
                        <i data-lucide="map-pin" class="w-4 h-4 text-indigo-500"></i> Ruangan / Lokasi
                    </label>
                    <input type="text" name="ruangan" value="{{ old('ruangan') }}" placeholder="Contoh: Ruang 101, Lab IPA" class="w-full neo-input py-3.5 px-5 font-bold text-sm">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 pt-4">
                <div class="space-y-3">
                    ...
                    <div class="flex gap-4 p-2 neo-pressed rounded-2xl">
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="semester" value="1" class="sr-only peer" {{ old('semester', '1') == '1' ? 'checked' : '' }}>
                        ...
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="semester" value="2" class="sr-only peer" {{ old('semester') == '2' ? 'checked' : '' }}>
                ...
                <div class="space-y-3">
                    <label class="flex items-center gap-2 text-xs font-black text-[var(--text-secondary)] uppercase tracking-widest ml-1">
                        <i data-lucide="activity" class="w-4 h-4 text-indigo-500"></i> Status Jadwal
                    </label>
            ...
            <div class="space-y-3">
                <label class="flex items-center gap-2 text-xs font-black text-[var(--text-secondary)] uppercase tracking-widest ml-1">
                    <i data-lucide="info" class="w-4 h-4 text-indigo-500"></i> Catatan Tambahan (Opsional)
                </label>
                <textarea name="keterangan" rows="3" placeholder="Informasi tambahan terkait jadwal..." class="w-full neo-input py-4 px-5 font-bold text-sm resize-none">{{ old('keterangan') }}</textarea>
            </div>

            <!-- Action Buttons -->
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-[var(--shadow-dark)]/10">
                <button type="reset" class="neo-btn px-8 py-4 text-[10px] font-black text-rose-500 uppercase tracking-widest hover:bg-rose-500 hover:text-white transition-all">
                    Reset Form
                </button>
