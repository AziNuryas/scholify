...
<body class="antialiased flex h-screen overflow-hidden">
    ...
    <aside class="w-[290px] bg-white h-full flex flex-col transition-all duration-300 z-20 border-r border-[#E0E5F2]">
        ...
        <div class="h-24 flex-none flex items-center px-6">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl overflow-hidden shadow-lg shadow-indigo-200 bg-gradient-to-br from-[#4318FF] to-[#868CFF] flex items-center justify-center">
                    <img src="{{ asset('images/scholify-logo.png') }}" 
                         alt="Scholify Logo" 
                         class="w-full h-full object-cover">
                </div>
                <div class="flex flex-col">
                    <span class="font-outfit font-extrabold text-xl tracking-tight text-[#2B3674] leading-tight">Scholify</span>
                    <span class="text-[9px] font-semibold text-[#A3AED0] tracking-wide uppercase">Belajar · Terhubung · Berkembang</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-4">
            <nav class="space-y-1.5 mt-4 font-outfit">
                ...
                <a href="{{ route('guru.dashboard') }}" class="nav-link flex items-center gap-3 px-4 py-3.5 rounded-xl {{ request()->routeIs('guru.dashboard') ? 'bg-[#4318FF] text-white font-semibold shadow-xl shadow-indigo-100 active sidebar-active' : 'text-[#A3AED0] hover:text-[#2B3674] hover:bg-[#F4F7FE] font-medium' }}">
                    <i class="fas fa-tachometer-alt text-xl nav-icon"></i>
                    <span>Dashboard</span>
                </a>
                
                <a href="{{ route('guru.jadwal') }}" class="nav-link flex items-center gap-3 px-4 py-3.5 rounded-xl {{ request()->routeIs('guru.jadwal') ? 'bg-[#4318FF] text-white font-semibold shadow-xl shadow-indigo-100 active sidebar-active' : 'text-[#A3AED0] hover:text-[#2B3674] hover:bg-[#F4F7FE] font-medium' }}">
                    <i class="fas fa-calendar-alt text-xl nav-icon"></i>
                    <span>Jadwal Kelas</span>
                </a>
                
                <a href="{{ route('guru.absensi') }}" class="nav-link flex items-center gap-3 px-4 py-3.5 rounded-xl {{ request()->routeIs('guru.absensi') ? 'bg-[#4318FF] text-white font-semibold shadow-xl shadow-indigo-100 active sidebar-active' : 'text-[#A3AED0] hover:text-[#2B3674] hover:bg-[#F4F7FE] font-medium' }}">
                    <i class="fas fa-user-check text-xl nav-icon"></i>
                    <span>Absensi</span>
                </a>
                
                <a href="{{ route('guru.nilai') }}" class="nav-link flex items-center gap-3 px-4 py-3.5 rounded-xl {{ request()->routeIs('guru.nilai') ? 'bg-[#4318FF] text-white font-semibold shadow-xl shadow-indigo-100 active sidebar-active' : 'text-[#A3AED0] hover:text-[#2B3674] hover:bg-[#F4F7FE] font-medium' }}">
                    <i class="fas fa-chart-line text-xl nav-icon"></i>
                    <span>Nilai & Rapor</span>
                </a>
                
                <a href="{{ route('guru.tugas') }}" class="nav-link flex items-center gap-3 px-4 py-3.5 rounded-xl {{ request()->routeIs('guru.tugas') ? 'bg-[#4318FF] text-white font-semibold shadow-xl shadow-indigo-100 active sidebar-active' : 'text-[#A3AED0] hover:text-[#2B3674] hover:bg-[#F4F7FE] font-medium' }} relative">
                    <i class="fas fa-book-open text-xl nav-icon"></i>
                    <span>Tugas Mandiri</span>
                </a>

                <a href="{{ route('guru.pengumuman') }}" class="nav-link flex items-center gap-3 px-4 py-3.5 rounded-xl {{ request()->routeIs('guru.pengumuman') ? 'bg-[#4318FF] text-white font-semibold shadow-xl shadow-indigo-100 active sidebar-active' : 'text-[#A3AED0] hover:text-[#2B3674] hover:bg-[#F4F7FE] font-medium' }}">
                ...
                <a href="{{ route('guru.profil') }}" class="nav-link flex items-center gap-3 px-4 py-3.5 rounded-xl {{ request()->routeIs('guru.profil') ? 'bg-[#4318FF] text-white font-semibold shadow-xl shadow-indigo-100 active sidebar-active' : 'text-[#A3AED0] hover:text-[#2B3674] hover:bg-[#F4F7FE] font-medium' }}">
    ...
    <main class="flex-1 flex flex-col h-full relative overflow-hidden">
        <header class="h-24 px-10 flex-none flex items-center justify-between glass-header sticky top-0 z-10">
            <div>
                <p class="text-[13px] text-[#A3AED0] font-semibold tracking-wide uppercase">Teacher Workspace</p>
                <h2 class="font-outfit font-extrabold text-2xl text-[#2B3674]">Halo, Selamat Mengajar! 👋</h2>
            </div>
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" @click.away="open = false" class="flex items-center gap-3 pl-3 border-l border-[#F4F7FE] hover:bg-[#F4F7FE]/50 p-1.5 rounded-2xl transition-all outline-none group">
                        <div class="text-right hidden sm:block">
                            <p class="font-bold text-sm text-[#2B3674] group-hover:text-[#4318FF] transition-colors leading-tight">Bapak Guru Budi</p>
                            <p class="text-[#A3AED0] text-[11px] font-medium uppercase tracking-tighter">Guru RPL</p>
                        </div>
                        <div class="relative">
                    ...
                    <div x-show="open" 
                        ...
                        <div class="px-4 py-2 border-bottom border-[#F4F7FE] mb-1">
                            <p class="text-[10px] font-bold text-[#A3AED0] uppercase tracking-widest">Akun Guru</p>
                        </div>

                        <a href="{{ route('guru.profil') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-[#2B3674] hover:bg-[#F4F7FE] hover:text-[#4318FF] transition-all font-semibold mx-2 rounded-xl">
